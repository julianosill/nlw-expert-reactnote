...
export function NoteCard({ note, onNoteDeleted }: NoteCardProps) {
  return (
    <Dialog.Root>
      <Dialog.Trigger className="rounded-md bg-slate-800 flex flex-col gap-3 text-left outline-none p-5 overflow-hidden relative hover:ring-2 hover:ring-slate-600 focus-visible:ring-2 focus-visible:ring-lime-400">
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed inset-0 md:inset-auto md:left-1/2 md:top-1/2 md:-translate-x-1/2 md:-translate-y-1/2 md:max-w-[640px] w-full md:h-[60vh] bg-slate-700 md:rounded-md flex flex-col outline-none overflow-hidden">
          ...
          <button
            type="button"
            className="w-full bg-slate-800 py-4 text-center text-sm outline-none font-medium text-red-400 group"
            onClick={() => onNoteDeleted(note.id)}
          >
            <span className="group-hover:underline"> Delete note</span>
          </button>
        </Dialog.Content>
      </Dialog.Portal>
    </Dialog.Root>
